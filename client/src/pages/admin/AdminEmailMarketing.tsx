...
export default function AdminEmailMarketing() {
  ...
  const tabs = [
    ...
    { id: 'campaigns' as Tab, label: 'Campanhas', icon: Send },
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Gerencie eventos, templates e envie emails para seus clientes
          </p>
        </div>
...
function DashboardTab() {
  ...
  const statCards = [
    ...
    { label: "Campanhas", value: stats?.totalCampaigns || 0, icon: Send, color: "text-purple-400" },
    { label: "Emails Enviados", value: stats?.totalEmailsSent || 0, icon: Mail, color: "text-amber-400" },
  ];
              ...
              return (
                <div key={event.id} className="flex items-center justify-between p-3 bg-accent/30 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-medium">{eventDate.toLocaleDateString('pt-BR')}</p>
...
function EventsTab() {
  ...
  const handleSendEmail = (event: any) => {
    if (!event.templateId) {
      toast.error("Este evento não tem template associado. Edite o evento e selecione um template.");
      return;
    }
    const template = templates?.find((t: any) => t.id === event.templateId);
    if (!template) {
      toast.error("Template não encontrado. Verifique se o template ainda existe.");
      return;
    }
            ...
            return (
              <div key={event.id} className={`flex items-center justify-between p-4 bg-card border border-border/50 rounded-xl hover:border-purple-500/30 transition-colors ${isPast ? 'opacity-60' : ''}`}>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm text-muted-foreground mr-2">{eventDate.toLocaleDateString('pt-BR')}</span>
                  <button
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-between">
        <p className="text-sm text-muted-foreground">
          Templates de email para usar nas campanhas. Use <code className="bg-accent px-1 rounded">{"{{nome}}"}</code> para o nome do cliente e <code className="bg-accent px-1 rounded">{"{{fotografo}}"}</code> para seu nome.
        </p>
      ...
      {showForm && (
        <div className="bg-card border border-purple-500/30 rounded-xl p-5 space-y-4">
          ...
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">Assunto do Email *</label>
            <input
              type="text"
              value={form.subject}
...
function CampaignsTab() {
  ...
  const { data: templates } = trpc.emailMarketing.listTemplates.useQuery();
  const { data: clientsList } = trpc.clients.list.useQuery();
  const createCampaign = trpc.emailMarketing.createCampaign.useMutation({ onSuccess: () => { refetch(); setShowForm(false); toast.success("Campanha criada!"); } });
  const sendCampaign = trpc.emailMarketing.sendCampaign.useMutation({ 
    onSuccess: (data) => { 
      refetch(); 
      toast.success(`Campanha enviada! ${data.sentCount} enviados, ${data.failedCount} falharam`); 
    },
    onError: (err) => toast.error(err.message),
  });
  const deleteCampaign = trpc.emailMarketing.deleteCampaign.useMutation({ onSuccess: () => { refetch(); toast.success("Campanha removida!"); } });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-between">
        <p className="text-sm text-muted-foreground">
          Crie e envie campanhas de email para seus clientes
        </p>
        <button
          onClick={() => setShowForm(!showForm)}
          className="flex items-center gap-2 px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-sm font-medium transition-colors"
      ...
      {showForm && (
        <div className="bg-card border border-purple-500/30 rounded-xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/50"
              >
                <option value={0}>Nenhum (escrever do zero)</option>
                {templates?.map((t: any) => (
                  <option key={t.id} value={t.id}>{t.name}</option>
                ))}
              </select>
          ...
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">Destinatários</label>
            <select
              value={form.recipientType}
              onChange={e => setForm({ ...form, recipientType: e.target.value })}
              className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/50"
            >
              <option value="all">Todos os clientes ({clientsList?.length || 0})</option>
              <option value="selected">Clientes selecionados</option>
            </select>
          </div>
          {form.recipientType === 'selected' && (
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">Selecione os clientes</label>
    ...
    <div className="space-y-4">
      ...
      {!logs || logs.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground bg-card border border-border/50 rounded-xl">
          ...
          <p className="text-xs mt-1">O histórico aparecerá aqui quando você enviar emails</p>
        </div>
      ) : (
                ...
                {logs.map((log: any) => (
                  <tr key={log.id} className="border-b border-border/30 hover:bg-accent/20">
                    <td className="px-4 py-3">
                      {log.status === 'sent' ? (
                        <span className="flex items-center gap-1 text-green-400 text-xs"><CheckCircle className="w-3.5 h-3.5" /> Enviado</span>
                      ) : log.status === 'failed' ? (
